...
#include "postprocesses/BlurComputePostProcess.h"
...
CMeshScene::CMeshScene()
{
	...
	{
		m_grayScalePostProcess = std::make_shared<CGrayScalePostProcess>();
		m_drawer.AddPostProcess(m_grayScalePostProcess);
	}
	{
		m_blurPostProcess = std::make_shared<CBlurComputePostProcess>();
		m_drawer.AddPostProcess(m_blurPostProcess);
	}
}
...
void CMeshScene::Update(double dt)
{
	...
	enabled = m_blurPostProcess->IsEnabled();
	ImGui::Checkbox("Blur Post Process", &enabled);
	m_blurPostProcess->SetEnabled(enabled);

	if(m_grayScalePostProcess)
	{
		enabled = m_grayScalePostProcess->IsEnabled();
		ImGui::Checkbox("Gray Scale Post Process", &enabled);
		m_grayScalePostProcess->SetEnabled(enabled);
	}
