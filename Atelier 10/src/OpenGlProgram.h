...
namespace OpenGl
{
	...
	public:
		...
		void SetUniformBlockBinding(const char* name, GLint value)
		{
			GLint uniformBinding = glGetUniformBlockIndex(m_handle, name);
			if(uniformBinding == GL_INVALID_INDEX)
			{
				printf("Warning: Uniform '%s' does not exist.\r\n", name);
				return;
			}
			glUniformBlockBinding(m_handle, uniformBinding, value);
		}
		...
		void SetUniform1i(const char* name, GLint value)
		{
			...
			glProgramUniform1i(m_handle, uniformLocation, value);
		}
